            ...
            for move_dir in all_dirs:
                if game_state.can_move_robot(rname, move_dir):
                    ...
                    if dest_tile.robot is None:
                        dest_tiles += [(move_dir, dest_tile)]

            # dest_tiles = sorted(dest_tiles, key=lambda x: x[1].row + x[1].col)
            # dest_tiles.reverse()

            final_tiles = []
            for move_dir, dest_tile in dest_tiles:
                distance = sqrt((dest_tile.row - start_point[0]) ** 2 + (dest_tile.col - start_point[1]) ** 2)
                final_tiles += [(move_dir, dest_tile, distance)]
            sorted_list = sorted(final_tiles, key=lambda x: x[2])
            selected_one = 0
            if len(sorted_list) > 0:
                selected_one = sorted_list[0]
                move_dir = selected_one[0]
            print(move_dir, selected_one)
