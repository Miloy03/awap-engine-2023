...
class BotPlayer(Player):
    ...
    def play_turn(self, game_state: GameState) -> None:
        ...
        if len(ally_tiles) > 0:
            ...
            if spawn_loc.state == TileState.MINING:
                spawn_type = RobotType.MINER
            elif spawn_loc.state == TileState.TERRAFORMABLE and spawn_loc.terraform < 3:
                spawn_type = RobotType.TERRAFORMER
            else:
                spawn_type = random.choice([RobotType.EXPLORER, RobotType.TERRAFORMER, RobotType.MINER])
            # spawn the robot
